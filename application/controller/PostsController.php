...
//    chi xu ly upload thoi. ke cmm
    ...
    public function updatelike()
    {
        ...
        if (isset($_POST["action"]) && $_POST["action"] == "updatelike") {
            ...
            $objId = $_POST["obj_id"];
            $userId = $_SESSION["user_id"];
            $statusLike = $post->statusLike($objId, $userId, $isAlbum);//cai nay luc cu nao cung null
            if ($statusLike === false) {
                 $post->likePost($isAlbum, $objId, $userId);
            } else {
                $post->updateLike(!$statusLike["active"], $userId, $objId, $isAlbum);
            }
        }
    }
