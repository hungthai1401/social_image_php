...
class Posts extends Controller
{
    ...
    public function likePost($isAlbum, $objId, $userId)
    {

//        var_dump($isAlbum); exit('tai anh thai');
        $isAlbum = $isAlbum == '0' ? false : true;
    ...
    public function statusLike($objId, $userId, $isAlbum)
    {
        $sql = "SELECT active FROM likes WHERE obj_id = :obj_id AND user_id = :user_id AND is_album = :is_album";
        try{
            $query = $this->db->prepare($sql);
            $parameters = array(
                ":obj_id" => $objId,
    ...
    public function updateLike($active, $userId, $objId, $isAlbum)
    {
        $sql = "UPDATE likes SET active = :active WHERE user_id = :user_id AND obj_id = :obj_id AND is_album = :is_album";
        try{
            $query = $this->db->prepare($sql);
            $parameters = array(
                ":active" => $active,
...
//            die(json_encode($query));
    ...
    public function getCommentImage($objId)
    {
