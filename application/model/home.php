...
class Home extends Controller
{
    ...
    public function getLikeAlbumsByUserId($userId, $objId)
    {
        $sql = "SELECT likes.* FROM likes INNER JOIN users ON likes.user_id = users.user_id WHERE likes.user_id = :user_id AND likes.obj_id = :obj_id AND is_album = 1";
        $query = $this->db->prepare($sql);
        $parameters = array(
            ":user_id" => $userId,
            ":obj_id" => $objId
